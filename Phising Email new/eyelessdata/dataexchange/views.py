...
def mailcount(request):
    unam=request.session['username']
    c.execute("select count(*) from message where sendto='"+unam+"'")
    data=c.fetchone()
    c.execute("select count(*) from message where sendto='"+unam+"' and status='spam'")
    data1=c.fetchone()
    ncnt=int(data[0])-int(data1[0])
    return render(request,"mailcount.html",{"total":data[0],"pcnt":data1[0],"ncnt":ncnt})
def blockmail(request):
    c.execute("select email_id from registration ")
    data=c.fetchall()
    msg=""
    if 'block' in request.POST:
        ...
        toa=request.POST["t1"]
        status="block"
        c.execute("insert into block (`from`,`to`,status)values('"+str(unam)+"','"+str(toa)+"','"+str(status)+"')")
        db.commit()
        msg="Blocked"
    if 'unblock' in request.POST:
        ...
        toa=request.POST["t1"]
        status="block"
        c.execute("delete from block where `from` ='"+str(unam)+"' and `to`='"+str(toa)+"'")
        db.commit()
        msg="unblocked"
    return render(request,"block.html",{"data":data,"msg":msg})
def compose(request):
    ...
    if(request.session['username']):
        ...
        for cd in data["URL"]:
            ...
            print(type(con) )   
            if con == cd:
                c.execute("update message set status='spam' where m_id='"+frm+"'")
                db.commit()
                cntn=75
                break
            if cc>=50:
        ...
        return HttpResponseRedirect("/login/")           
    return render(request,"compose.html",{"frm1":frm1,"sub":sub,"con":con,"s":s,"data2":data2,"cnt":cnt,"cntn":cntn})
def draft1(request):
    det=[]
    if(request.session['username']):
        s1="sent"
...
def inbox(request):
    if(request.session['username']):
        ...
        c1=c.fetchone()
        c.execute("select `from`,date,subject,content,sendto,m_id from message where sendto='"+unam+"'  and `from` not in (select `to` from block where `from`='"+str(unam)+"')" )
        data=c.fetchall()
